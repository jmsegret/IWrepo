...
C-elsewhere. Thus, one does not need to run setup upon restart
      ...
      write(nhist) ((tubc(i,j),tbbc(i,j),i=1,nxpp),j=1,ny)
...
c Soliton velocity/density and gradient fields (includes pressure gradient)
      write(nhist) umax ! TAK ADDED
      write(nhist) solks
      write(nhist) ((umwv(i,k),wmwv(i,k),i=1,nxpp),k=1,nz)
      write(nhist) ((dumdx(i,k),dumdz(i,k),i=1,nxpp),k=1,nz)
      write(nhist) ((dwmdx(i,k),dwmdz(i,k),i=1,nxpp),k=1,nz)
      write(nhist) ((rhomwv(i,k),i=1,nxpp),k=1,nz)
      write(nhist) ((drhomdx(i,k),drhomdz(i,k),i=1,nxpp),k=1,nz)
!     write(nhist) (rhobar(k),k=1,nz)      ! TAK ADDED
!     write(nhist) (drhobardz(k),k=1,nz)   ! TAK ADDED
!     write(nhist) (d2rhobardz2(k),k=1,nz) ! TAK ADDED
...
c    +                 x(i),y(j),z(k)      
            ...
            enddo 
         enddo
 1003    format(1x,3(i5,1x),1x,7(e9.3,1x),1x)
