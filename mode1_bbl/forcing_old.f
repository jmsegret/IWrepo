...
C-Assume for all practical purposes that the density is 1
C-at the bottom of the domain.
      parameter (dt= 0.0270295880980098, a=0.986111111111111,
     >             b=0.0138888888888889, dn=1.)
      z0 = 0.2625

      arg = 4.*(z-z0)/(dt*0.45)
      arg0 = 4./(dt*0.45)

      c1 = -rho0*brunt**2./grav
      c2 = (tanh(arg) - tanh(-arg0*z0))/arg0
      c3 = (log ( cosh(arg) ) - log( cosh(arg0*z0) ))/arg0

      rhomean = 1.+c1*(a*c2 + b*z - b*dn*c3)
c     rhomean = 1.+ rhograd(z,zlen,grav,rho0,brunt)*z
...
C-Dt represents non-dimensional thickness of thermocline
     ...
     >             b=0.0138888888888889, dn=1.)
      ...
      sech2 = 1./(cosh(arg)**2.)
      tanh2 = 1. - dn*tanh(arg)
      bruntz = brunt*sqrt( a*sech2 + b*tanh2 )
!      bruntz = brunt
...
c xkcrit,xmcrit , and xomega  are dimensional quantities 
     ...
     >                zcen,xbcrit,zdim,umax)
c- rampu-up of source terms A.M.A. 03-03-2008 
           implicit none 
           real Tch,sigmw,sinf,cosf,Tinterv,Ttot,veldim,forceund 
           real x,z,t,brunt,fzlocp,PI,phit,forceu,
     >                xacrit,xkcrit,xmcrit,xomega,
     >                zcen,xbcrit,zdim,umax,fzloc,fxgaus

       sinf = sin( xkcrit*x + xmcrit*z - xomega*t )
       cosf = cos( xkcrit*x + xmcrit*z - xomega*t ) 
       veldim = umax*umax/zdim  
       forceund=(-(xacrit*xmcrit/xkcrit)*fzloc(z,zcen,zdim,xbcrit)
     >    *cosf)-((xacrit/xkcrit)*fzlocp(z,zcen,zdim,xbcrit)*sinf)
       forceu = veldim*forceund*phit(t,xomega)*
     >    fxgaus(x,0.2,0.1014,1./4.2919,2) !two wavelengths
!     >    fxgaus(x,6.0,8.0,1./4.2919,2) !four  wavelengths
!       function fxgaus(x,xcen,zdim,xfrac,p)
        return
        end
...
c-w-momentum
     ...
     >                zcen,xbcrit,zdim,umax)
         implicit none
          real Tch,sigmw,sinf,cosf,Tinterv,Ttot,veldim,forcewnd
          real x,z,t,brunt,fzlocp,PI,phit,forcew,
     >                xacrit,xkcrit,xmcrit,xomega,
     >                zcen,xbcrit,zdim,umax,fzloc,fxgaus

        cosf = cos( xkcrit*x + xmcrit*z - xomega*t )
        veldim = umax*umax/zdim         
        forcewnd = xacrit*fzloc(z,zcen,zdim,xbcrit)*cosf  
        forcew = veldim*forcewnd*phit(t,xomega)*
     >    fxgaus(x,0.2,0.1014,1./4.2919,2) !two wavelengths
!     >    fxgaus(x,3.2,4.0,1./4.2919,2)
!     >    fxgaus(x,3.0,2.0,1./4.2919,2) !two wavelengths
!  !   >    fxgaus(x,6.0,8.0,1./4.2919,2) !four  wavelengths
      return
      end
...
c-Careful here: I begin with S&R's original equation (2.32).
c-The radial frequency in A/omega is non-dimensional. Must
c-take account of this here
     ...
     >                  brunt,rho0,grav)
         ...
         implicit none 
         real Tch,sigmw,sinf,cosf,Tinterv,Ttot,veldim,forcerhond
         real xomegandim, rhodim,forcerho,fzloc,rho0
         real x,z,t,fzlocp,brunt,grav,
     >             phit, xacrit,xkcrit,xmcrit,xomega,
     >              PI, zcen,xbcrit,zdim,umax,fxgaus
        PI=4.0*atan(1.0)
        sinf = sin( xkcrit*x + xmcrit*z - xomega*t )
        xomegandim = (xomega*zdim)/(umax)
        rhodim =umax*brunt*brunt*rho0/grav
        forcerhond = -(xacrit/xomegandim)*fzloc(z,zcen,zdim,xbcrit)
     >               *sinf
        forcerho = rhodim*forcerhond*phit(t,xomega)*
     >    fxgaus(x,0.2,0.1014,1./4.2919,2) !two wavelengths
!     >    fxgaus(x,3.2,4.0,1./4.2919,2) two wavelengths
!     >    fxgaus(x,6.0,8.0,1./4.2919,2) !four  wavelengths
!     >    fxgaus(x,3.0,2.0,1./4.2919,2) !two wavelengths
        ...
        return
         ...
         implicit none
         real t,xomega,phit,Tch,Tinterv,PI,Ttot

         phit = 1.0
   
c         PI=4.0*atan(1.0)
cc         Tch=0.5*2.0*PI/xomega
c         Tinterv=2.0*PI/xomega 
c         Tch=0.25*Tinterv
c         Ttot=0.75*Tinterv

cc         Ttot=Tinterv+Tch
c         if (t < Tinterv) phit=1.0
c         if (t > Tinterv) phit=0.0

!D.C. Fritts

c           Tinterv=6.0*PI/xomega 
c                  if (t < Tinterv) phit=t/Tinterv
c                  if(t > Tinterv) phit=1.0

c               phit=1.0
c          if (t.lt.0.25*Tinterv)phit=sin(2*PI*t/Tinterv)
c          if ((t.ge.0.25*Tinterv).and.(t.lt.75*Tinterv))phit=1
c          if ((t.ge.75*Tinterv).and.(t.lt.Tinterv))
c     >        phit=sin(2*PI*(t-0.5*Tinterv)/Tinterv) 
c          if (t.gt.Tinterv) phit=0

C sudden turn on, gradual turn off
c           if (t < Tinterv)phit=1.0
c           if ((t > Tinterv).and.(t < 1.25*Tinterv))
c     >        phit=sin(2*PI*(t-0.75*Tinterv)/Tinterv) 
c           if (t > 1.25*Tinterv) phit=0.0

  
c          if (t <= Tch)phit= t/Tch !exp(-0.5*((t-Tch)/Tch)**2)
c          if (t > Tch.and.t <= Ttot) phit=1.0
c          if (t >Ttot.and.t <= Tinterv) phit=(-t/Tch+4.0) !exp(-10.0*((t-Ttot)/Tch)**2)
c          if(t > Tinterv) phit=0.0
