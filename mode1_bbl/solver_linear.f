...
c         timeout = t
          ...
          timeout = ianimc
          fmt2='(i1,a4)'
          if (nint(timeout).ge.10)  fmt2='(i2,a4)'
          if (nint(timeout).ge.100) fmt2='(i3,a4)'
          if (nint(timeout).ge.1000) fmt2='(i4,a4)'
...
!     >               rhomean(z(1),zlen,grav,rho0,brunt) )
     ...
     >         (-grav*dtdz(i,j,k)/rho0),rhomean(zz,zlen,grav,rho0,brunt)
...
!-PD-6/8/12: Added this one little parameter to enable turning
!-the non-linear terms on or off. 
!-0=off
!-1-on
      parameter(inonlinearon=0)
c
      include 'comflow.h'
      include 'commatrix.h'
      include 'comtime.h'
...
!-----------------------------------------------------------
!-PD-6/8/12: Proceed with following only if non-linear terms
!-are "on".
!-----------------------------------------------------------

      if (inonlinearon.eq.1) then !--N.L. TERM ON
...
!-PD: Moved this outside of loop right now, as it is
!-purely linear (6/8/12)
!-Add GRAVITY term in physical space
!           tfluct = (temp(i,j,k) - tempmean(k))
!           sw(i,j,k) = sw(i,j,k) + 
!    >                 ((bruntz(zz,zlen,brunt)**2.)/
!    >                  rhograd(zz,zlen,grav,rho0,brunt))*
!    >                  temp(i,j,k)
!          
!           if ((i.eq.nxhp).and.(j.eq.nyh))  write(*,*) z(k),tfluct
!    >write(*,*) su(i,j,k),sv(i,j,k),sw(i,j,k)
          enddo
        enddo
       enddo
...
!-PD-6/8/12:
!-Add GRAVITY term in physical space
!-This has been now removed from the loop above
...
!-----------------------------------------------------------
!-PD-6/8/12: In what follows, soliton terms are only relevant
!-if wave-velocity field is non-zero. Otherwise, these have
!-no contribution
!-----------------------------------------------------------
...
!-----------------------------------------------
!-PD-6/8/12:
!-If nonlinear terms are ON, proceed and
!-compute d(wT)/dz. Otherwise, set this term to
!-zero
!-----------------------------------------------

      if (inonlinearon.eq.1) then
...
!--------------------------------------------
!-PD-6/8/12: In what follows, contribution of
!-mean gradient is accounted for. d(wT)/dz is
!-only accounted for if N-L terms are ON.
!--------------------------------------------
