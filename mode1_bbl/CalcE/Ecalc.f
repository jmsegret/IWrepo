...
!      REAL :: xx,zz,u,w,rhop,rho
      INTEGER :: II,JJ,N1,N2,skip,ianimc,t
      INTEGER :: ks,kloc!,i,j,k
      ...
      do j=1,JJ
         do i=1,II   
            ...
            elseif (i.ne.I) then
               ...
               Ew(i,j)=1/2*w(i,j)*w(i,j)*(xx(i+1,j)-xx(i,j))
            elseif (i.eq.I) then
...
!      function integrcol(amat,outint,wg,ybar)
       
!      real :: integrcol,amat,outint,ybar,sum,wg
!      Integer :: nzm,k,nzloc

!      sum = 0.0

!      do k=1,nzloc
!          sum = sum + wg(k-1)*amat(k)
!      enddo
